...
public class RequireGet {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
				List<String> stc = FileUtil.FileToList("C:/Users/强胜/Desktop/dataCrawler/usnews/schoolurl.txt");
				List<String> rank = FileUtil.FileToList("C:/Users/强胜/Desktop/dataCrawler/usnews/rank.txt");
				List<String> urllist = new ArrayList<String>();
				BufferedWriter bw = FileUtil.FileWriter("C:/Users/强胜/Desktop/dataCrawler/usnews/require.json",true);
				Map<String,String> map = new HashMap<String,String>();
				for(String xx : rank)
					map.put(xx.split("____")[0],xx.split("____")[1]);
					
				for(int i = 7 ; i < stc.size() ; i++){
					System.out.println("process__"+(i));
					try{
						bw.write(process(stc.get(i).split("____")[0],map.get(stc.get(i).split("____")[0]),stc.get(i).split("____")[1]));
						bw.newLine();
						bw.flush();
					}catch(Exception e){
						e.printStackTrace();
			...
			private static String process(String name, String id ,String each) {
				// TODO Auto-generated method stub
				String result = "-1";
				Document html = null;
				try {
					...
					map.put("rank",id);
					try{
						map.put("applications",getHttp(each+"/applying/applications"));
					}catch(Exception e){
						e.printStackTrace();
					}
					try{
						map.put("requirements",getHttp(each+"/applying/requirements"));
					}catch(Exception e){
						e.printStackTrace();
					}
					try{
						map.put("entering-class-stats",getHttp(each+"/applying/entering-class-stats"));
					}catch(Exception e){
						e.printStackTrace();
					}
					try{
						map.put("transferring",getHttp(each+"/applying/transferring"));
					}catch(Exception e){
						e.printStackTrace();
					}
					try{
						map.put("international-students",getHttp(each+"/applying/international-students"));
					}catch(Exception e){
						e.printStackTrace();
					}
